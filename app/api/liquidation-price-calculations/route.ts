...
export async function POST(req: Request) {
	...
	try {
		...
		if (session?.user.plan === 'free') {
			const calcCount = await db.liquidationPriceCalculation.count({
				where: {
		...
		const { entryPrice, leverageRatio } = liquidationPriceFormDataSchema.parse(body.formData);
		const name = calculationNameStringSchema.parse(body.name);

		const calculation = await db.liquidationPriceCalculation.create({
			data: {
				name,
				formData: {
					entryPrice,
...
export async function GET() {
	...
	try {
		...
		const calculations = await db.liquidationPriceCalculation.findMany({
			where: {
				userId: session?.user.id,
			},
		});
