...
export async function POST(req: Request) {
	...
	try {
		...
		if (session?.user.plan === 'free') {
			const calcCount = await db.presentValueCalculation.count({
				where: {
		...
		const { discountRate, duration, durationMultiplier, startingBalance } =
			presentValueFormDataSchema.parse(body.formData);
		const name = calculationNameStringSchema.parse(body.name);

		const calculation = await db.presentValueCalculation.create({
			data: {
				name,
				formData: {
					discountRate,
...
export async function GET() {
	...
	try {
		...
		const calculations = await db.presentValueCalculation.findMany({
			where: {
				userId: session?.user.id,
			},
		});
