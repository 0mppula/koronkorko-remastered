...
export async function POST(req: Request) {
	...
	try {
		...
		if (session?.user.plan === 'free') {
			const calcCount = await db.compoundInterestCalculation.count({
				where: {
		...
		const {
			...
			interestRate,
			startingBalance,
		} = compoundInterestFormDataSchema.parse(body.formData);
		const name = calculationNameStringSchema.parse(body.name);

		const calculation = await db.compoundInterestCalculation.create({
			data: {
				name,
				formData: {
					compoundFrequency,
...
export async function GET() {
	...
	try {
		...
		const calculations = await db.compoundInterestCalculation.findMany({
			where: {
				userId: session?.user.id,
			},
		});
